...
(function() {
  ...
  Extends[OPKEY] = {
    $in: function(item, arr) {
      return __indexOf.call(arr, item) >= 0;
    },
    $eq: _.isEqual,
    $neq: function() {
  ...
  check = function(obj, rule) {
    ...
    _wrap = function(target, handler, addtion) {
      ...
      rts = _.map(target, function(tar) {
        return handler(tar, addtion);
      });
      rt = _.reduce(rts, function(a, b) {
        return a && b;
      });
      return [rt, single ? rts[0] : rts];
    };
    _check = function(tar, key) {
      var cusRule, handler, rts, toCheck, _i, _len, _ref1;
      toCheck = tar[key];
      for (_i = 0, _len = CUS.length; _i < _len; _i++) {
        cusRule = CUS[_i];
        handler = Extends[CUKEY][cusRule];
        rt = _wrap(toCheck, handler, rule[cusRule])[0];
        if (!rt) {
          err = cusRule;
          return false;
        }
      ...
      if (!method) {
        ...
        _ref1 = _wrap(toCheck, method, data), rt = _ref1[0], rts = _ref1[1];
        if (OPTS.format) {
          tar[key] = rts;
        }
        if (!rt) {
